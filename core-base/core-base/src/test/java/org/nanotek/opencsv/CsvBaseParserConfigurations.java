...
M extends BaseBean<?,?>> {
    ...
    private final Map<String, CsvFileItemConcreteStrategy<T,S,P,M>> csvStrategies;
    ...
    public CsvBaseParserConfigurations(@Autowired CsvFileConfigurations<T,S,P,M> csvFileConfigurations) {
        ...
        if (rawStrategies != null && !rawStrategies.isEmpty()) {
            ...
            rawStrategies.forEach((name, strategy) -> {
                try {
                    // Call afterPropertiesSet on each strategy to ensure reader is set up
                    strategy.afterPropertiesSet();
                    System.out.println("  - Initialized strategy: " + name);
                } catch (Exception e) {
                    System.err.println("  - Error initializing strategy " + name + ": " + e.getMessage());
                    e.printStackTrace();
                }
            });
            this.csvStrategies = Collections.unmodifiableMap(rawStrategies); // Make it unmodifiable
            ...
            this.csvStrategies = Collections.emptyMap();
        }
    }
     ...
     */
    public Map<String, CsvFileItemConcreteStrategy<T,S,P,M>> getCsvStrategies() {
     ...
     */
    public Optional<CsvFileItemConcreteStrategy<T,S,P,M>> getStrategy(String strategyName) {
