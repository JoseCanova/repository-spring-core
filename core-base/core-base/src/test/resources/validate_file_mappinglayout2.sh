...
if [ -z "$action" ]; then
    ...
    extracted_data=$(awk -v target_block="$target_block_name" '
    BEGIN {
        in_target_block_scope = 0;
        in_base_map_section = 0;
        file_location_val = "";
        file_name_val = "";
        base_map_lines = 0;
        found_target_block_and_data = 0;
    }

    /^[[:alnum:]_]+:/ {
        current_block_val = substr($0, 1, index($0, ":") - 1);
        if (current_block_val == target_block) {
            in_target_block_scope = 1;
            file_location_val = "";
            file_name_val = "";
            base_map_lines = 0;
            in_base_map_section = 0;
        } else {
            if (in_target_block_scope == 1 && in_base_map_section == 0) {
                found_target_block_and_data = 1;
                exit;
            }
            in_target_block_scope = 0;
        }
    ...
    {
        if (in_target_block_scope == 1) {
            if ($0 ~ /^[[:blank:]]{2,}fileLocation:[[:blank:]]*(.*)$/) {
                file_location_val = substr($0, index($0, ":") + 2);
                sub(/^[[:blank:]]*/, "", file_location_val);
            } else if ($0 ~ /^[[:blank:]]{2,}fileName:[[:blank:]]*(.*)$/) {
                file_name_val = substr($0, index($0, ":") + 2);
                sub(/^[[:blank:]]*/, "", file_name_val);
            } else if ($0 ~ /^[[:blank:]]{2,}baseMap:$/) {
                in_base_map_section = 1;
            } else if (in_base_map_section == 1) {
                if ($0 ~ /^[[:blank:]]{4,}[^[:blank:]]/) {
                    base_map_lines++;
                } else {
                    in_base_map_section = 0;
    ...
    END {
        if (in_target_block_scope == 1 && (file_location_val != "" || file_name_val != "" || base_map_lines > 0)) {
            found_target_block_and_data = 1;
        }

        if (found_target_block_and_data == 1) {
