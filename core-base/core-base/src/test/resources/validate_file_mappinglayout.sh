...
if [ -z "$action" ]; then
    ...
    /^[[:alnum:]_]+:/ {
        ...
        if (current_block_val == target_block) {
            ...
            # If we were in the target block and a new block starts, it means we are done
            if (in_target_block == 1) {
                exit;
            }
            in_target_block = 0; # Not in target block
        }
        next;
    }

    {
        if (in_target_block == 1) {
            if ($0 ~ /^[[:blank:]]{2,}fileLocation:[[:blank:]]*(.*)$/) {
                ...
                in_base_map = 1;
            } else if (in_base_map == 1) {
                # Check if the line is still part of baseMap (indented by at least 4 blanks)
                if ($0 ~ /^[[:blank:]]{4,}[^[:blank:]]/) {
                    base_map_lines++;
                } else {
                    # End of baseMap section for the target block
                    in_base_map = 0;
                    # Exit after finding all info for the target block
                    exit;
                }
            }
        }
