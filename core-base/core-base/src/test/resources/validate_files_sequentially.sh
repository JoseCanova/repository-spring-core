...
find "$SCAN_DIRECTORY" -type f -name "*.yml" -print0 | while IFS= read -r -d $'\0' yml_file; do
    ...
    # Execute the validate_file_mappinglayout4.sh script
    # We use '|| true' to ensure the loop continues even if the validation script returns a non-zero exit code (error).
    "$VALIDATE_SCRIPT" "$FULL_PATH" "$BLOCK_NAME" "validate"
    
    # Check the exit status of the last command (the validation script call)
    if [ $? -eq 0 ]; then
